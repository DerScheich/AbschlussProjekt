...
class PlayUtils:
    ...
    async def safe_send(self, ctx, content: str):
        """Schickt eine Antwort, egal ob normaler oder Slash-Command."""
        try:
    ...
    def _ensure_playlist_link(self, link: str) -> str:
        """Macht aus youtube.com/watch?v=ABC&list=XYZ -> youtube.com/playlist?list=XYZ."""
        import urllib.parse
        parsed = urllib.parse.urlparse(link)
    ...
    async def _add_single(self, ctx: commands.Context, link: str):
        """Fügt ein einzelnes YouTube-Video (oder ein Download-Format) zur Queue hinzu."""
        loop = asyncio.get_event_loop()
        try:
    ...
    async def _add_spotify_single(self, ctx: commands.Context, query: str):
        """Sucht passendes YouTube-Video für Spotify-Track per Query und fügt es als Single ein."""
        import urllib.parse, urllib.request, re
    ...
    async def _peek_next_spotify_track(self, entry: dict, ctx) -> tuple | None:
        """
        Ermittelt (Tracktitel, Query) für den nächsten Spotify-Track,
        ohne den current_index zu erhöhen (nur Vorschau).
        """
        loop = asyncio.get_event_loop()
        try:
    ...
    async def _get_next_spotify_track(self, entry: dict, ctx) -> tuple | None:
        """
        Holt den nächsten Track aus der Spotify-Playlist und erhöht den current_index.
        Gibt (Tracktitel, Query) zurück oder None, falls das Ende erreicht ist.
        """
        loop = asyncio.get_event_loop()
        try:
            result = await loop.run_in_executor(
    ...
    async def skip_tracks(self, ctx: commands.Context, amount: int) -> list:
        """
        Überspringt (amount - 1) Tracks aus der Queue.
        Der aktuell spielende Track wird durch voice_client.stop() beendet.
        Gibt eine Liste der übersprungenen Titel zurück.
        """
        skipped = []
        ...
        while extra > 0 and q:
            ...
            if isinstance(item, dict) and item.get("type") == "spotify_playlist":
                ...
                if res is None:
                    # Playlist zu Ende, also entfernen
                    q.pop(0)
                    continue
                title, _ = res
    ...
    async def play_next(self, ctx: commands.Context):
        # Entferne den aktuell spielenden Song
